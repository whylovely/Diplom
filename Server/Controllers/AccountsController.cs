...
/// <summary>
/// CRUD счетов пользователя. Все запросы фильтруются по UserId, мягкое удаление через
/// флаг IsDeleted. При попытке удалить счёт с ненулевым балансом или с проводками
/// возвращает 400 — для принудительного удаления есть параметр <c>?force=true</c>.
/// Уникальность имени счёта проверяется в рамках одного пользователя (среди не удалённых).
/// </summary>
[ApiController]
[Authorize]
[Route("api/accounts")]
